...
public:
    ...
    bool serialReaction();  // 밀림으로 인한 연쇄 작용
};
...
bool isWall(int nr, int nc, int curH, int curW){    // 이동할 좌표가 포함된 영역에 벽이 있는가 벽인가
    int upperR = nr+curH,
        upperC = nc+curW;
...
bool Knight::serialReaction(){  // 밀림으로 인한 연쇄 작용. 벽으로 인해 이동이 불가능하면 false 반환
    return true;
}
